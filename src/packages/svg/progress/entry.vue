...
export default {
    ...
    methods: {
        ...
        getAxisPath(rects, side = 'left') {
            ...
            let aside1 = side === 'left' ? this.tickConfig.margin.x - offsetX : this.tickConfig.width - this.tickConfig.margin.x + offsetX
            let aside2 = side === 'left' ? aside1 + 5 : aside1 - 5
            let d = ''
            let i = 0
            ...
            let lastPos = rects[i - 1].pos * 2 - rects[i - 2].pos
        ...
        getTransfrom(rects, rate) {
            let x = this.tickConfig.width - this.tickConfig.margin.x
            let sinX = rects[Math.min(Math.floor(rate), this.tickConfig.num - 1)].pos + this.offset.y + 4.5
            let y = this.tickConfig.height - this.tickConfig.margin.y - sinX
