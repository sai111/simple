        ...
        <svg
            ...
            <defs>
                <filter
                    ...
                    <feComposite :in="isGradient?'offsetBlur5':'colorBlur5'" in2="SourceAlpha" operator="out" result="comp5" />
                    <!-- 颜色2 -->
                    ...
                    <feComposite :in="isGradient?'offsetBlur10':'colorBlur10'" in2="SourceAlpha" operator="out" result="comp10" />
                    <!-- 颜色3 -->
                    ...
                    <feComposite :in="isGradient?'offsetBlur20':'colorBlur20'" in2="SourceAlpha" operator="out" result="comp20" />
                    <!-- 融合 -->
                    <feMerge>
            ...
            <g class="progress-bottom">
                ...
                <template v-else>
                    <rect
                        ...
                        :width="rectWidth"
                        :height="rectHeight"
                        :transform="`translate(${getX(item.pos)}, ${getY(item.pos)}) ${isVertical ? 'skewY': 'skewX'}(${angle})`"
                        :fill="baseColor||item.color" />
...
export default {
    ...
    computed: {
        ...
        absSkewLength() {
            let angleL = Math.abs(Math.tan(this.angle * Math.PI / 180))
            let w = this.width - this.margin.x * 2
            let h = this.height - this.margin.y * 2
        ...
        unit() {
            let result
            let numPadding = this.num + (this.num - 1) * this.itemGap
            if (this.isVertical) {
            ...
            for (let i = 0; i < this.num; i++) {
                ...
                if (this.gradientRGB) {
                    ...
                    for (let j = 0; j < this.gradientRGB.length && flag; j++) {
                        let stop = this.gradientRGB[j]
                        if (rate < stop.value) {
                            if (j === 0) {
                                flag = false
                                rect.color = `rgb(${stop.r}, ${stop.g}, ${stop.b})`
                            } else {
                                let lastStop = this.gradientRGB[j - 1]
                                let k1 = (stop.value - rate) / (stop.value - lastStop.value)
                                let k2 = 1 - k1
                                flag = false
    ...
    methods: {
        ...
        lengthTick() {
            if (this.destryedFlag) return false
            let now = Date.now()
            let speed = this.chargeSpeed * 1000
            // 时间范围
            let timeDiff = ((now - this.currentTime) || 15) / speed
            this.currentTime = now
            let isAdd = this.currentRate < this.rateNum ? 1 : -1
